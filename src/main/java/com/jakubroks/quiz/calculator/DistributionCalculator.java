...
public abstract class DistributionCalculator {

    public abstract Map<Difficulty, Integer> calculate(int total, Map<Difficulty, Integer> weights);

    final Map<Difficulty, Integer> calculateBaseDistributionWithFloor(int total, Map<Difficulty, Integer> weights) {
    ...
    final int countAssignedQuestions(Map<Difficulty, Integer> distribution) {
        return distribution.values().stream()
                .mapToInt(Integer::intValue)
                .sum();
    }

    final void validateTotal(int total) {
        if (total < 1) {
    ...
    final void validateWeights(Map<Difficulty, Integer> weights) {
